...
console.log(splitMe("Io mangio la pasta"));
...
console.log(onlyLetters("Ciao123"));
...
console.log(isThisAnEmail("email123.com"));
console.log(isThisAnEmail("email123@gmail.com"));
...
function howManyDays(data) {
  ...
  const tempoDiff = oggi - data2;
  const giorniDiff = Math.floor(tempoDiff / (1000 * 60 * 60 * 24));
  return giorniDiff;
}

console.log(howManyDays("2023-10-01"));
console.log(howManyDays("2024-12-25"));
...
function isTodayMyBirthday() {
  let oggi = new Date();
  const compleanno = new Date(oggi.getFullYear(), 10, 10);
  return (
...
function newestMovie(movies) {
  const newestYear = Math.max(...movies.map((movie) => parseInt(movie.Year)));
  return movies.find((movie) => parseInt(movie.Year) === newestYear);
}
